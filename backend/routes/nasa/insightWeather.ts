...
import { respondWithCache, buildCacheKey, INSIGHT_WEATHER_CACHE_TTL, NASA_API_KEY } from '../../services/nasaApiHelper';
...
router.get('/insight_weather', async (req: Request, res: Response) => {
  ...
  if (respondWithCache(res, cache, cacheKey, INSIGHT_WEATHER_CACHE_TTL)) return;
  ...
  try {
    ...
    respondWithCache(res, cache, cacheKey, INSIGHT_WEATHER_CACHE_TTL, data);
    res.json(data);
  } catch (err: any) {
    if (err.response) {
